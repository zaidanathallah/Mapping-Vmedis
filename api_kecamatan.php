...
function getKecamatanData($accessToken, $city_codes) {
    $url = "https://api-satusehat-stg.dto.kemkes.go.id/masterdata/v1/districts?city_codes=" . urlencode($city_codes);
...
if (isset($_POST['IDKOTASATUSEHAT']) && !empty($_POST['IDKOTASATUSEHAT'])) {
    $IDKOTASATUSEHAT = $_POST['IDKOTASATUSEHAT'];
    error_log("Received IDKOTASATUSEHAT: " . $IDKOTASATUSEHAT);

    $accessToken = getCachedAccessToken();  // Gunakan token yang dicache
    if ($accessToken) {
        $KecamatanData = getKecamatanData($accessToken, $IDKOTASATUSEHAT); // Panggil fungsi getCityData dengan ID Provinsi
    ...
    error_log("IDKOTASATUSEHAT tidak diberikan.");
    echo json_encode(['error' => "IDKOTASATUSEHAT tidak diberikan."]);
}
